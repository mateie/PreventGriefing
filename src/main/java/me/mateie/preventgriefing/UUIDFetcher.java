...
class UUIDFetcher
{
    ...
    public void call() throws Exception
    {
        ...
        PreventGriefing.AddLogEntry("UUID conversion process started.  Please be patient - this may take a while.");

        PreventGriefing.AddLogEntry("Mining your local world data to save calls to Mojang...");
        OfflinePlayer[] players = PreventGriefing.instance.getServer().getOfflinePlayers();
        ...
        PreventGriefing.AddLogEntry("Checking local server data to get correct casing for player names...");
        for (int i = 0; i < names.size(); i++)
        {
            ...
            {
                PreventGriefing.AddLogEntry(name + " --> " + correctCasingName);
                names.set(i, correctCasingName);
            }
        }

        //look for local uuid's first
        PreventGriefing.AddLogEntry("Checking local server data for UUIDs already seen...");
        for (int i = 0; i < names.size(); i++)
        {
            String name = names.get(i);
            ...
            if (uuid != null)
            {
                PreventGriefing.AddLogEntry(name + " --> " + uuid.toString());
                names.remove(i--);
            }
        }
        ...
        if (PreventGriefing.instance.getServer().getOnlineMode())
        {
            ...
            names.removeIf(name ->
            {
                ...
                PreventGriefing.AddLogEntry(String.format("Cannot convert invalid name: %s", name));
                return true;
            });

            PreventGriefing.AddLogEntry("Calling Mojang to get UUIDs for remaining unresolved players (this is the slowest step)...");
                ...
                do
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (e.getMessage().contains("429"))
                        {
                            ...
                            {
                                PreventGriefing.AddLogEntry("Batch size " + PROFILES_PER_REQUEST + " seems too large.  Looking for a workable batch size...");
                            ...
                            {
                                PreventGriefing.AddLogEntry("Mojang says we're sending requests too fast.  Will retry every 30 seconds until we succeed...");
                                Thread.sleep(30000);
                            }
                        }
                        else
        ...
        {
            PreventGriefing.AddLogEntry("Generating offline mode UUIDs for remaining unresolved players...");
