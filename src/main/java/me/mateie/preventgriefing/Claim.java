...
import me.mateie.preventgriefing.events.ClaimPermissionCheckEvent;
...
public class Claim
{
    ...
    public void removeSurfaceFluids(Claim exclusionClaim)
    {
        ...
        if (!PreventGriefing.instance.creativeRulesApply(this.lesserBoundaryCorner)) return;
        ...
        if (lesser.getWorld().getEnvironment() == Environment.NORMAL)
            seaLevel = PreventGriefing.instance.getSeaLevel(lesser.getWorld());
            ...
            @Nullable Event event)
    {
        ...
        if (uuid.equals(this.getOwnerID())
                || PreventGriefing.instance.dataStore.getPlayerData(uuid).ignoreClaims
        ...
        if (permission == ClaimPermission.Build)
        {
            ...
            PlayerData playerData = PreventGriefing.instance.dataStore.getPlayerData(uuid);
            if (playerData.inPvpCombat())
            {
                return () -> PreventGriefing.instance.dataStore.getMessage(Messages.NoBuildPvP);
            }
        ...
        return () ->
        {
            String reason = PreventGriefing.instance.dataStore.getMessage(permission.getDenialMessage(), this.getOwnerName());
            if (hasBypassPermission(player, permission))
                reason += "  " + PreventGriefing.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
            return reason;
        };
    }
    ...
    public String getOwnerName()
    {
        ...
        if (this.ownerID == null)
            return PreventGriefing.instance.dataStore.getMessage(Messages.OwnerNameForAdminClaims);

        return PreventGriefing.lookupPlayerName(this.ownerID);
    }
    ...
    public String allowMoreEntities(boolean remove)
    {
        ...
        if (!PreventGriefing.instance.creativeRulesApply(this.getLesserBoundaryCorner())) return null;
        ...
        if (maxEntities == 0) return PreventGriefing.instance.dataStore.getMessage(Messages.ClaimTooSmallForEntities);
        ...
        if (totalEntities >= maxEntities)
            return PreventGriefing.instance.dataStore.getMessage(Messages.TooManyEntitiesInClaim);
    ...
    public String allowMoreActiveBlocks()
    {
        ...
        if (maxActives == 0)
            return PreventGriefing.instance.dataStore.getMessage(Messages.ClaimTooSmallForActiveBlocks);
        ...
        if (totalActives >= maxActives)
            return PreventGriefing.instance.dataStore.getMessage(Messages.TooManyActiveBlocksInClaim);
    ...
    public long getPlayerInvestmentScore()
    {
        ...
        boolean creativeMode = PreventGriefing.instance.creativeRulesApply(lesserBoundaryCorner);
        ...
        for (int x = this.lesserBoundaryCorner.getBlockX(); x <= this.greaterBoundaryCorner.getBlockX(); x++)
        {
            for (int z = this.lesserBoundaryCorner.getBlockZ(); z <= this.greaterBoundaryCorner.getBlockZ(); z++)
            {
                ...
                for (; y < PreventGriefing.instance.getSeaLevel(this.lesserBoundaryCorner.getWorld()) - 5; y++)
                {
