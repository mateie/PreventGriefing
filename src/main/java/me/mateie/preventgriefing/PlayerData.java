...
public class PlayerData
{
    ...
    public boolean inPvpCombat()
    {
        ...
        if (elapsed > PreventGriefing.instance.config_pvp_combatTimeoutSeconds * 1000) //X seconds
        {
    ...
    {
        int remainingBlocks = this.getAccruedClaimBlocks() + this.getBonusClaimBlocks() + PreventGriefing.instance.dataStore.getGroupBonusBlocks(this.playerID);
    ...
    private void loadDataFromSecondaryStorage()
    {
        ...
        PlayerData storageData = PreventGriefing.instance.dataStore.getPlayerDataFromStorage(this.playerID);
        ...
        if (this.accruedClaimBlocks == null)
        {
            if (storageData.accruedClaimBlocks != null)
            {
                ...
                if (PreventGriefing.instance.config_advanced_fixNegativeClaimblockAmounts && (this.accruedClaimBlocks < PreventGriefing.instance.config_claims_initialBlocks))
                {
                    this.accruedClaimBlocks = PreventGriefing.instance.config_claims_initialBlocks;
                }
    ...
    public Vector<Claim> getClaims()
    {
        if (this.claims == null)
        {
            ...
            DataStore dataStore = PreventGriefing.instance.dataStore;
            ...
            int totalBlocks = this.accruedClaimBlocks + this.getBonusClaimBlocks() + PreventGriefing.instance.dataStore.getGroupBonusBlocks(this.playerID);
            if (PreventGriefing.instance.config_advanced_fixNegativeClaimblockAmounts && totalBlocks < totalClaimsArea)
            {
                OfflinePlayer player = PreventGriefing.instance.getServer().getOfflinePlayer(this.playerID);
                PreventGriefing.AddLogEntry(player.getName() + " has more claimed land than blocks available.  Adding blocks to fix.", CustomLogEntryTypes.Debug, true);
                PreventGriefing.AddLogEntry(player.getName() + " Accrued blocks: " + this.getAccruedClaimBlocks() + " Bonus blocks: " + this.getBonusClaimBlocks(), CustomLogEntryTypes.Debug, true);
                PreventGriefing.AddLogEntry("Total blocks: " + totalBlocks + " Total claimed area: " + totalClaimsArea, CustomLogEntryTypes.Debug, true);
                for (Claim claim : this.claims)
                {
                    if (!claim.inDataStore) continue;
                    PreventGriefing.AddLogEntry(
                            PreventGriefing.getfriendlyLocationString(claim.getLesserBoundaryCorner()) + " // "
                                    + PreventGriefing.getfriendlyLocationString(claim.getGreaterBoundaryCorner()) + " = "
                                    + claim.getArea()
                ...
                PreventGriefing.AddLogEntry("New accrued blocks: " + this.accruedClaimBlocks, CustomLogEntryTypes.Debug, true);

                //Recalculate total blocks (accrued + bonus + permission group bonus)
                totalBlocks = this.accruedClaimBlocks + this.getBonusClaimBlocks() + PreventGriefing.instance.dataStore.getGroupBonusBlocks(this.playerID);
                PreventGriefing.AddLogEntry("New total blocks: " + totalBlocks, CustomLogEntryTypes.Debug, true);
                ...
                if (totalBlocks < totalClaimsArea)
                {
                    ...
                    PreventGriefing.AddLogEntry("Accrued blocks weren't enough. Adding " + bonusBlocksToAdd + " bonus blocks.", CustomLogEntryTypes.Debug, true);
                }
                PreventGriefing.AddLogEntry(player.getName() + " Accrued blocks: " + this.getAccruedClaimBlocks() + " Bonus blocks: " + this.getBonusClaimBlocks() + " Group Bonus Blocks: " + PreventGriefing.instance.dataStore.getGroupBonusBlocks(this.playerID), CustomLogEntryTypes.Debug, true);
                //Recalculate total blocks (accrued + bonus + permission group bonus)
                totalBlocks = this.accruedClaimBlocks + this.getBonusClaimBlocks() + PreventGriefing.instance.dataStore.getGroupBonusBlocks(this.playerID);
                PreventGriefing.AddLogEntry("Total blocks: " + totalBlocks + " Total claimed area: " + totalClaimsArea, CustomLogEntryTypes.Debug, true);
                PreventGriefing.AddLogEntry("Remaining claim blocks to use: " + this.getRemainingClaimBlocks() + " (should be 0)", CustomLogEntryTypes.Debug, true);
            }
        }
    ...
    public int getAccruedClaimBlocksLimit()
    {
        if (this.AccruedClaimBlocksLimit < 0)
            return PreventGriefing.instance.config_claims_maxAccruedBlocks_default;
