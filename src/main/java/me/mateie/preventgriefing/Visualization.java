...
import com.preventgriefing.events.BoundaryVisualizationEvent;
...
public class Visualization
{
    ...
    public static void Revert(Player player)
    {
        if (player != null && player.isOnline())
            PreventGriefing.instance.dataStore.getPlayerData(player.getUniqueId()).setVisibleBoundaries(null);
    }
     ...
     */
    @Deprecated(forRemoval = true, since = "16.18")
    public static Visualization FromClaim(Claim claim, int height, me.mateie.preventgriefing.VisualizationType visualizationType, Location locality)
    {
        ...
        visualization.boundaries.addAll(
                claim.children.stream()
                        .map(child -> new Boundary(child, com.preventgriefing.visualization.VisualizationType.SUBDIVISION))
     ...
     */
    @Deprecated(forRemoval = true, since = "16.18")
    public void addClaimElements(Claim claim, int height, me.mateie.preventgriefing.VisualizationType visualizationType, Location locality)
    {
    ...
    public void addClaimElements(Location min, Location max, Location locality, int height, BlockData cornerBlockData, BlockData accentBlockData, int STEP) {
        this.boundaries.add(new Boundary(new BoundingBox(min, max), me.mateie.preventgriefing.VisualizationType.ofBlockData(accentBlockData)));
    }
     ...
     */
    @Deprecated(forRemoval = true, since = "16.18")
    public static Visualization fromClaims(Iterable<Claim> claims, int height, me.mateie.preventgriefing.VisualizationType type, Location locality)
    {
        Visualization visualization = new Visualization();
