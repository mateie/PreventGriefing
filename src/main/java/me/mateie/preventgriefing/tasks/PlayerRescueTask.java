...
public class PlayerRescueTask implements Runnable
{
    ...
    public void run()
    {
        ...
        {
            PreventGriefing.sendMessage(player, TextMode.Err, Messages.RescueAbortedMoved);
            return;
        }
        ...
        {
            this.destination = PreventGriefing.instance.ejectPlayer(this.player);
        }
        else
        {
        ...
        PreventGriefing.AddLogEntry("Rescued trapped player " + player.getName() + " from " + PreventGriefing.getfriendlyLocationString(this.location) + " to " + PreventGriefing.getfriendlyLocationString(this.destination) + ".");
    }
}
