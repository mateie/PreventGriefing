...
import me.mateie.preventgriefing.events.AccrueClaimBlocksEvent;
...
public class DeliverClaimBlocksTask implements Runnable
{
    ...
    public DeliverClaimBlocksTask(Player player, PreventGriefing instance)
    {
        this.player = player;
    ...
    public void run()
    {
        ...
        try
        {
            ...
            if (isIdle)
            {
                ...
                {
                    PreventGriefing.AddLogEntry(player.getName() + " wasn't active enough to accrue claim blocks this round.", CustomLogEntryTypes.Debug, true);
            ...
            {
                PreventGriefing.AddLogEntry(player.getName() + " claim block delivery was canceled by another plugin.", CustomLogEntryTypes.Debug, true);
            ...
            PreventGriefing.AddLogEntry("Delivering " + event.getBlocksToAccrue() + " blocks to " + player.getName(), CustomLogEntryTypes.Debug, true);
        ...
        {
            PreventGriefing.AddLogEntry("Problem delivering claim blocks to player " + player.getName() + ":");
            e.printStackTrace();
        }
    }
}
