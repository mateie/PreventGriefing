...
public class WelcomeTask implements Runnable
{
    ...
    public void run()
    {
        ...
        PreventGriefing.sendMessage(player, TextMode.Instr, Messages.AvoidGriefClaimLand);
        ...
        if (PreventGriefing.instance.config_claims_supplyPlayerManual)
        {
            ...
            String editToolName = PreventGriefing.instance.config_claims_modificationTool.name().replace('_', ' ').toLowerCase();
            String infoToolName = PreventGriefing.instance.config_claims_investigationTool.name().replace('_', ' ').toLowerCase();
