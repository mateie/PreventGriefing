...
public class EntityCleanupTask implements Runnable
{
    ...
    public void run()
    {
        ...
        for (World world : PreventGriefing.instance.getServer().getWorlds())
        {
        ...
        for (World world : worlds)
        {
            ...
            for (; j < entities.size() && j < k; j++)
            {
                ...
                {
                    Claim claim = PreventGriefing.instance.dataStore.getClaimAt(entity.getLocation(), false, cachedClaim);
                    if (claim != null)
                    {
                        cachedClaim = claim;
                    }
                ...
                if (remove)
                {
                    PreventGriefing.AddLogEntry("Removing entity " + entity.getType().name() + " @ " + entity.getLocation(), CustomLogEntryTypes.Debug, true);
                    entity.remove();
                }
            }
        }
        ...
        List<Claim> claims = PreventGriefing.instance.dataStore.claims;
        ...
        PreventGriefing.instance.getServer().getScheduler().scheduleSyncDelayedTask(PreventGriefing.instance, task, 20L * 60 * 1);
    }
}
