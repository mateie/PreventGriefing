...
import me.mateie.preventgriefing.BlockSnapshot;
    ...
    public void run()
    {
        ...
        for (int x = 1; x < this.snapshots.length - 1; x++)
        {
            for (int z = 1; z < this.snapshots[0][0].length - 1; z++)
            {
                for (int y = this.miny; y < this.snapshots[0].length; y++)
                {
                    ...
                    {
                        Claim claim = PreventGriefing.instance.dataStore.getClaimAt(blockUpdate.location, false, cachedClaim);
                        if (claim != null)
                        {
                            cachedClaim = claim;
                            break;
        ...
        for (Entity entity : entities)
        {
            if (!(entity instanceof Player || entity instanceof Animals))
            {
                ...
                if (!(entity instanceof Hanging) || PreventGriefing.instance.dataStore.getClaimAt(entity.getLocation(), false, null) == null)
                {
