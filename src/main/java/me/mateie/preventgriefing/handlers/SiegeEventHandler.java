    ...
    public void onClaimPermissionCheck(ClaimPermissionCheckEvent event)
    {
        ...
        if (event.getRequiredPermission() == ClaimPermission.Edit)
        {
            if (claim.siegeData != null)
                event.setDenialReason(() -> PreventGriefing.instance.dataStore.getMessage(Messages.NoModifyDuringSiege));
            return;
        }
        ...
        if (event.getRequiredPermission() == ClaimPermission.Inventory)
        {
            ...
            PreventGriefing.instance.dataStore.tryExtendSiege(player, claim);

            if (claim.siegeData != null)
                event.setDenialReason(() -> PreventGriefing.instance.dataStore.getMessage(Messages.NoContainersSiege, claim.siegeData.attacker.getName()));
        ...
        if (broken != null)
        {
            ...
            if (!PreventGriefing.instance.config_siege_blocks.contains(broken))
                event.setDenialReason(() -> PreventGriefing.instance.dataStore.getMessage(Messages.NonSiegeMaterial));
            else if (player.getUniqueId().equals(claim.ownerID))
                event.setDenialReason(() -> PreventGriefing.instance.dataStore.getMessage(Messages.NoOwnerBuildUnderSiege));
            return;
        }

        // No building while under siege.
        if (claim.siegeData != null)
            event.setDenialReason(() -> PreventGriefing.instance.dataStore.getMessage(Messages.NoBuildUnderSiege, claim.siegeData.attacker.getName()));
