...
import me.mateie.preventgriefing.PendingItemProtection;
...
import me.mateie.preventgriefing.events.PreventPvPEvent;
import me.mateie.preventgriefing.events.ProtectDeathDropsEvent;
...
public class EntityEventHandler implements Listener
{
    ...
    public EntityEventHandler(DataStore dataStore, PreventGriefing plugin)
    {
    ...
    public void onEntityFormBlock(EntityBlockFormEvent event)
    {
        ...
        if (entity.getType() == EntityType.PLAYER)
        {
            ...
            String noBuildReason = PreventGriefing.instance.allowBuild(player, event.getBlock().getLocation(), event.getNewState().getType());
    ...
    {
        if (!PreventGriefing.instance.config_endermenMoveBlocks && event.getEntityType() == EntityType.ENDERMAN)
        {
            event.setCancelled(true);
        }
        else if (!PreventGriefing.instance.config_silverfishBreakBlocks && event.getEntityType() == EntityType.SILVERFISH)
        {
            event.setCancelled(true);
        }
        else if (!PreventGriefing.instance.config_rabbitsEatCrops && event.getEntityType() == EntityType.RABBIT)
        {
            event.setCancelled(true);
        }
        else if (!PreventGriefing.instance.config_claims_ravagersBreakBlocks && event.getEntityType() == EntityType.RAVAGER)
        {
            event.setCancelled(true);
        }
        // All other handling depends on claims being enabled.
        else if (PreventGriefing.instance.config_claims_worldModes.get(event.getBlock().getWorld()) == ClaimsMode.Disabled)
        {
            return;
        }
        ...
        else if (event.getEntityType() == EntityType.WITHER)
        {
            ...
            if (claim == null || !claim.areExplosivesAllowed || !PreventGriefing.instance.config_blockClaimExplosions)
            {
                event.setCancelled(true);
            }
        }
        ...
        else if (event.getTo() == Material.DIRT && event.getBlock().getType() == Material.FARMLAND)
        {
            ...
            else
            {
                ...
                if (PreventGriefing.instance.allowBreak(player, block, block.getLocation()) != null)
                {
                    event.setCancelled(true);
                }
            }
        ...
        else if (event.getEntity() instanceof Vehicle && !event.getEntity().getPassengers().isEmpty())
        {
            ...
            {
                Block block = event.getBlock();
                if (PreventGriefing.instance.allowBreak((Player) driver, block, block.getLocation()) != null)
                {
                    event.setCancelled(true);
                }
            }
        ...
        else if (event.getEntityType() == EntityType.FALLING_BLOCK)
        {
            ...
            {
                entity.setMetadata("GP_FALLINGBLOCK", new FixedMetadataValue(PreventGriefing.instance, block.getLocation()));
            }
            ...
            else
            {
                ...
                if (originalLocation.getBlockX() != newLocation.getBlockX() || originalLocation.getBlockZ() != newLocation.getBlockZ())
                {
                    ...
                    if (PreventGriefing.instance.config_claims_worldModes.get(newLocation.getWorld()) == ClaimsMode.Creative)
                    {
                        event.setCancelled(true);
                        entity.remove();
                        return;
    ...
    private void handleProjectileChangeBlock(EntityChangeBlockEvent event, Projectile projectile)
    {
        ...
        if (claim == null)
        {
            ...
            {
                if (!PreventGriefing.instance.config_fireDestroys || !PreventGriefing.instance.config_fireSpreads)
                    event.setCancelled(true);
                return;
            }
        ...
        if (shooter instanceof Player)
        {
            ...
            if (denial != null)
            {
                ...
                PreventGriefing.sendMessage((Player) shooter, TextMode.Err, denial.get());
                event.setCancelled(true);
            }
    ...
    private boolean isBlockSourceInClaim(ProjectileSource projectileSource, Claim claim)
    {
        return projectileSource instanceof BlockProjectileSource &&
                PreventGriefing.instance.dataStore.getClaimAt(((BlockProjectileSource) projectileSource).getBlock().getLocation(), false, claim) == claim;
    }
    ...
    {
        if (!PreventGriefing.instance.config_zombiesBreakDoors) event.setCancelled(true);
    }
    ...
    public void onEntityInteract(EntityInteractEvent event)
    {
        ...
        {
            if (!PreventGriefing.instance.config_creaturesTrampleCrops)
            {
                event.setCancelled(true);
            }
            else
    ...
    void handleExplosion(Location location, Entity entity, List<Block> blocks)
    {
        ...
        if (!PreventGriefing.instance.claimsEnabledForWorld(world)) return;
        ...
        boolean applySurfaceRules = world.getEnvironment() == Environment.NORMAL && ((isCreeper && PreventGriefing.instance.config_blockSurfaceCreeperExplosions) || (!isCreeper && PreventGriefing.instance.config_blockSurfaceOtherExplosions));

        //special rule for creative worlds: explosions don't destroy anything
        if (PreventGriefing.instance.creativeRulesApply(location))
        {
        ...
        for (Block block : blocks)
        {
            ...
            if (claim != null && claim.siegeData != null)
            {
                ...
                boolean breakable = PreventGriefing.instance.config_siege_blocks.contains(material);
            ...
            if (claim == null)
            {
                if (!applySurfaceRules || block.getLocation().getBlockY() < PreventGriefing.instance.getSeaLevel(world) - 7)
                {
                    explodedBlocks.add(block);
                }
            }
    ...
    public void onItemSpawn(ItemSpawnEvent event)
    {
        ...
        if (PreventGriefing.instance.creativeRulesApply(event.getLocation()))
        {
            event.setCancelled(true);
        }

        //if item is on watch list, apply protection
        ArrayList<PendingItemProtection> watchList = PreventGriefing.instance.pendingItemWatchList;
        ...
        for (int i = 0; i < watchList.size(); i++)
        {
            ...
            newItem.setMetadata("GP_ITEMOWNER", new FixedMetadataValue(PreventGriefing.instance, pendingProtection.owner));
    ...
    public void onExpBottle(ExpBottleEvent event)
    {
        ...
        if (PreventGriefing.instance.creativeRulesApply(event.getEntity().getLocation()))
        {
    ...
    public void onEntitySpawn(CreatureSpawnEvent event)
    {
        ...
        if (!PreventGriefing.instance.creativeRulesApply(event.getLocation())) return;
    ...
    public void onEntityDeath(EntityDeathEvent event)
    {
        ...
        if (!PreventGriefing.instance.claimsEnabledForWorld(entity.getWorld())) return;

        //special rule for creative worlds: killed entities don't drop items or experience orbs
        if (PreventGriefing.instance.creativeRulesApply(entity.getLocation()))
        {
        ...
        boolean isPvPWorld = PreventGriefing.instance.pvpRulesApply(world);
        if ((isPvPWorld && PreventGriefing.instance.config_lockDeathDropsInPvpWorlds) ||
                (!isPvPWorld && PreventGriefing.instance.config_lockDeathDropsInNonPvpWorlds))
        {
            ...
            if (!protectionEvent.isCancelled())
            {
                ...
                {
                    PreventGriefing.instance.pendingItemWatchList.add(
    ...
    public void onHangingBreak(HangingBreakEvent event)
    {
        ...
        if (!PreventGriefing.instance.claimsEnabledForWorld(event.getEntity().getWorld())) return;
        ...
        String noBuildReason = PreventGriefing.instance.allowBuild(playerRemover, event.getEntity().getLocation(), Material.AIR);
        if (noBuildReason != null)
        {
            event.setCancelled(true);
            PreventGriefing.sendMessage(playerRemover, TextMode.Err, noBuildReason);
        }
    }
    ...
    {
        //don't track in worlds where claims are not enabled
        if (!PreventGriefing.instance.claimsEnabledForWorld(event.getBlock().getWorld())) return;

        //FEATURE: similar to above, placing a painting requires build permission in the claim

        //if the player doesn't have permission, don't allow the placement
        String noBuildReason = PreventGriefing.instance.allowBuild(event.getPlayer(), event.getEntity().getLocation(), Material.PAINTING);
        if (noBuildReason != null)
        {
            event.setCancelled(true);
            PreventGriefing.sendMessage(event.getPlayer(), TextMode.Err, noBuildReason);
            return;
        }

        //otherwise, apply entity-count limitations for creative worlds
        else if (PreventGriefing.instance.creativeRulesApply(event.getEntity().getLocation()))
        {
            ...
            {
                PreventGriefing.sendMessage(event.getPlayer(), TextMode.Err, noEntitiesReason);
                event.setCancelled(true);
                return;
            }
        }
    ...
    private void handleEntityDamageEvent(EntityDamageEvent event, boolean sendErrorMessagesToPlayers)
    {
        ...
        if (event.getEntity() instanceof Horse && !PreventGriefing.instance.config_claims_protectHorses) return;
        if (event.getEntity() instanceof Donkey && !PreventGriefing.instance.config_claims_protectDonkeys) return;
        if (event.getEntity() instanceof Mule && !PreventGriefing.instance.config_claims_protectDonkeys) return;
        if (event.getEntity() instanceof Llama && !PreventGriefing.instance.config_claims_protectLlamas) return;
        ...
        if (event.getEntity() instanceof Tameable && !PreventGriefing.instance.pvpRulesApply(event.getEntity().getWorld()))
        {
        ...
        if (damageSource != null)
        {
            ...
            if (damageSource.getType() == EntityType.AREA_EFFECT_CLOUD && event.getEntityType() == EntityType.PLAYER && PreventGriefing.instance.pvpRulesApply(event.getEntity().getWorld()))
            {
                Player damaged = (Player) event.getEntity();
                PlayerData damagedData = PreventGriefing.instance.dataStore.getPlayerData(damaged.getUniqueId());

                //case 1: recently spawned
                if (PreventGriefing.instance.config_pvp_protectFreshSpawns && damagedData.pvpImmune)
                {
                    event.setCancelled(true);
                    return;
                }
                ...
                {
                    Claim damagedClaim = PreventGriefing.instance.dataStore.getClaimAt(damaged.getLocation(), false, damagedData.lastClaim);
                    if (damagedClaim != null)
                    {
                        damagedData.lastClaim = damagedClaim;
                        if (PreventGriefing.instance.claimIsPvPSafeZone(damagedClaim))
                        {
        ...
        if (firework != null && event.getEntityType() == EntityType.PLAYER && !PreventGriefing.instance.pvpRulesApply(attacker.getWorld()))
        {
        ...
        if (attacker != null && event.getEntityType() == EntityType.PLAYER && PreventGriefing.instance.pvpRulesApply(attacker.getWorld()))
        {
            ...
            if (attacker != defender)
            {
                ...
                if (PreventGriefing.instance.config_pvp_protectFreshSpawns)
                {
                    if (defenderData.pvpImmune)
                    {
                        event.setCancelled(true);
                        if (sendErrorMessagesToPlayers)
                            PreventGriefing.sendMessage(attacker, TextMode.Err, Messages.ThatPlayerPvPImmune);
                        return;
                    }
                    ...
                    if (attackerData.pvpImmune)
                    {
                        event.setCancelled(true);
                        if (sendErrorMessagesToPlayers)
                            PreventGriefing.sendMessage(attacker, TextMode.Err, Messages.CantFightWhileImmune);
                        return;
                    }
                }

                //FEATURE: prevent players from engaging in PvP combat inside land claims (when it's disabled)
                if (PreventGriefing.instance.config_pvp_noCombatInPlayerLandClaims || PreventGriefing.instance.config_pvp_noCombatInAdminLandClaims)
                {
                    ...
                    if (!attackerData.ignoreClaims)
                    {
                        if (attackerClaim != null && //ignore claims mode allows for pvp inside land claims
                                ...
                                PreventGriefing.instance.claimIsPvPSafeZone(attackerClaim))
                        {
                        ...
                        if (defenderClaim != null &&
                                ...
                                PreventGriefing.instance.claimIsPvPSafeZone(defenderClaim))
                        {
                            ...
                            if (!pvpEvent.isCancelled())
                            {
                                event.setCancelled(true);
                                if (sendErrorMessagesToPlayers)
                                    PreventGriefing.sendMessage(attacker, TextMode.Err, Messages.PlayerInPvPSafeZone);
                            }
                            return;
                        }
                    }
        ...
        if (event instanceof EntityDamageByEntityEvent)
        {
            ...
            if (event.getEntityType() == EntityType.PLAYER)
            {
                ...
                if (damager != null && damager instanceof Tameable)
                {
                    ...
                    if (pet.isTamed() && pet.getOwner() != null)
                    {
                        ...
                        PlayerData defenderData = PreventGriefing.instance.dataStore.getPlayerData(event.getEntity().getUniqueId());
                    ...
                    || subEvent.getEntityType() == EntityType.ENDER_CRYSTAL)
            {
                ...
                if (subEvent.getEntityType() == EntityType.VILLAGER && !PreventGriefing.instance.config_claims_protectCreatures)
                    return;
                ...
                if (claim != null)
                {
                    ...
                    if (failureReason != null)
                    {
                        event.setCancelled(true);
                        if (sendErrorMessagesToPlayers)
                            PreventGriefing.sendMessage(attacker, TextMode.Err, failureReason.get());
                        return;
                    }
                }
            }
            ...
            if (((subEvent.getEntity() instanceof Creature || subEvent.getEntity() instanceof WaterMob) && PreventGriefing.instance.config_claims_protectCreatures))
            {
                ...
                if (subEvent.getEntity() instanceof Tameable)
                {
                    ...
                    if (tameable.isTamed() && tameable.getOwner() != null)
                    {
                        ...
                        if (attacker != null)
                        {
                            ...
                            if (!PreventGriefing.instance.pvpRulesApply(subEvent.getEntity().getLocation().getWorld()) || (PreventGriefing.instance.config_pvp_protectPets && subEvent.getEntityType() != EntityType.WOLF))
                            {
                                OfflinePlayer owner = PreventGriefing.instance.getServer().getOfflinePlayer(ownerID);
                                String ownerName = owner.getName();
                                if (ownerName == null) ownerName = "someone";
                                String message = PreventGriefing.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity, ownerName);
                                if (attacker.hasPermission("griefprevention.ignoreclaims"))
                                    message += "  " + PreventGriefing.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
                                if (sendErrorMessagesToPlayers)
                                    PreventGriefing.sendMessage(attacker, TextMode.Err, message);
                            ...
                            else if (tameable.getType() == EntityType.WOLF)
                            {
                                if (!tameable.getOwner().equals(attacker))
                                {
                                    ...
                                    event.setCancelled(true);
                                    String ownerName = PreventGriefing.instance.getServer().getOfflinePlayer(ownerID).getName();
                                    String message = PreventGriefing.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity, ownerName);
                                    if (attacker.hasPermission("griefprevention.ignoreclaims"))
                                        message += "  " + PreventGriefing.instance.dataStore.getMessage(Messages.IgnoreClaimsAdvertisement);
                                    if (sendErrorMessagesToPlayers)
                ...
                if (claim != null)
                {
                    ...
                    else if (!(event.getEntity().getWorld().getPVP() && event.getEntity().getType() == EntityType.WOLF))
                    {
                        ...
                        if (sendErrorMessagesToPlayers)
                        {
                            ...
                            override = () ->
                            {
                                String message = PreventGriefing.instance.dataStore.getMessage(Messages.NoDamageClaimedEntity, claim.getOwnerName());
                        ...
                        if (noContainersReason != null)
                        {
                            ...
                            if (sendErrorMessagesToPlayers)
                            {
                                PreventGriefing.sendMessage(attacker, TextMode.Err, noContainersReason.get());
                            }
                            event.setCancelled(true);
                        }
    ...
    private boolean handleBlockExplosionDamage(EntityDamageEvent event)
    {
        ...
        Claim claim = PreventGriefing.instance.dataStore.getClaimAt(entity.getLocation(), false, null);
    ...
    public void onCrossbowFireWork(EntityShootBowEvent shootEvent)
    {
        ...
        {
            shootEvent.getProjectile().setMetadata("GP_FIREWORK", new FixedMetadataValue(PreventGriefing.instance, shootEvent.getEntity()));
        }
    }
    ...
    public void onEntityDamageMonitor(EntityDamageEvent event)
    {
        ...
        if (!PreventGriefing.instance.pvpRulesApply(defender.getWorld())) return;
    ...
    public void onVehicleDamage(VehicleDamageEvent event)
    {
        ...
        if (!PreventGriefing.instance.config_claims_preventTheft) return;

        //input validation
        if (event.getVehicle() == null) return;

        //don't track in worlds where claims are not enabled
        if (!PreventGriefing.instance.claimsEnabledForWorld(event.getVehicle().getWorld())) return;
        ...
        for (PotionEffect effect : effects)
        {
                        ...
                        if (claim != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (noContainersReason != null)
                                {
                                    ...
                                    if (!messagedPlayer)
                                    {
                                        PreventGriefing.sendMessage(thrower, TextMode.Err, noContainersReason.get());
                                        messagedPlayer = true;
                                    }
                                }
                            }
            ...
            for (LivingEntity effected : event.getAffectedEntities())
            {
                ...
                if (effected.getType() != EntityType.PLAYER) continue;
                    ...
                    //FEATURE: prevent players from engaging in PvP combat inside land claims (when it's disabled)
                else if (PreventGriefing.instance.config_pvp_noCombatInPlayerLandClaims || PreventGriefing.instance.config_pvp_noCombatInAdminLandClaims)
                {
                    ...
                    if (attackerClaim != null && PreventGriefing.instance.claimIsPvPSafeZone(attackerClaim))
                    {
                        attackerData.lastClaim = attackerClaim;
                        ...
                        if (!pvpEvent.isCancelled())
                        {
                            ...
                            if (!messagedPlayer)
                            {
                                PreventGriefing.sendMessage(thrower, TextMode.Err, Messages.CantFightWhileImmune);
                                messagedPlayer = true;
                            }
                        }
                        continue;
                    ...
                    if (defenderClaim != null && PreventGriefing.instance.claimIsPvPSafeZone(defenderClaim))
                    {
                        defenderData.lastClaim = defenderClaim;
                        ...
                        if (!pvpEvent.isCancelled())
                        {
                            ...
                            if (!messagedPlayer)
                            {
                                PreventGriefing.sendMessage(thrower, TextMode.Err, Messages.PlayerInPvPSafeZone);
                                messagedPlayer = true;
                            }
                        }
                    }
